...
class TrainerFix(object):
    ...
    def __init__(self, info, args):
        super(TrainerFix, self).__init__()
        ...
        if self.params.pretrained_file is not None:
            print('Use pretrained file: {}'.format(self.params.pretrained_file))
            ...
            # self.load_model('./weights-best.pt')
    ...
    def init_optimizer(self, num_epochs):
        """
        Initialize optimizer and scheduler and filter fix parts
        """
        for name, value in self.model.named_parameters():
            if name in ['module.0.logits.weight','module.0.logits.bias']:
                value.requires_grad = False
        params = filter(lambda p: p.requires_grad, self.model.parameters())
        self.optimizer = torch.optim.SGD(params, lr=self.params.lr, weight_decay=self.params.weight_decay, 
    ...
    def load_model(self, path, load_opt=True):
        ...
        fix_dict = {k:checkpoint['model_state_dict'][k] for k in checkpoint['model_state_dict'] if k in ['module.0.logits.weight','module.0.logits.bias']}
        model_dict = self.model.state_dict()
        model_dict.update(fix_dict)
