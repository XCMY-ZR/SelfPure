#The following code is written by Ray at 2023/09/02
...
def perturb_iterative(xvar, yvar, predict, nb_iter, eps, eps_iter, loss_fn, delta_init=None, minimize=False, ord=np.inf, 
                      clip_min=0.0, clip_max=1.0, HN_regular=False,noise_eps=8/255):
    """
    ...
    for ii in range(nb_iter):
        ...
        if HN_regular:
            x_in = xvar + delta 
            x_in = x_in + noise_eps * torch.randn_like(x_in)
            n_outputs = predict(clamp(x_in,clip_min,clip_max))
            loss = loss + torch.norm(F.softmax(n_outputs) - F.softmax(outputs),1)
...
class HNPGDAttack(Attack, LabelMixin):
    """
    ...
    def __init__(
            ...
            ord=np.inf, targeted=False, rand_init_type='uniform', HN_regular=False, noise_eps=8/255):
        super(HNPGDAttack, self).__init__(predict, loss_fn, clip_min, clip_max)
        self.eps = eps
    ...
    def perturb(self, x, y=None):
        ...
        x_adv, r_adv = perturb_iterative(
            ...
            minimize=self.targeted, ord=self.ord, clip_min=self.clip_min, clip_max=self.clip_max, delta_init=delta,HN_regular=self.HN_regular,noise_eps=self.noise_eps
        )

        return x_adv.data, r_adv.data


class HNLinfPGDAttack(HNPGDAttack):
    """
    ...
    def __init__(
            self, predict, loss_fn=None, eps=0.3, nb_iter=40, eps_iter=0.01, rand_init=True, clip_min=0., clip_max=1.,
            targeted=False, rand_init_type='uniform',HN_regular = False,noise_eps=8/255):
        ord = np.inf
        super(HNLinfPGDAttack, self).__init__(
            predict=predict, loss_fn=loss_fn, eps=eps, nb_iter=nb_iter, eps_iter=eps_iter, rand_init=rand_init, 
            clip_min=clip_min, clip_max=clip_max, targeted=targeted, ord=ord, rand_init_type=rand_init_type,HN_regular=HN_regular,noise_eps=noise_eps)


class HNL2PGDAttack(HNPGDAttack):
    """
    ...
    def __init__(
            ...
            targeted=False, rand_init_type='uniform',HN_regular = False,noise_eps=8/255):
        ord = 2
        super(HNL2PGDAttack, self).__init__(
            predict=predict, loss_fn=loss_fn, eps=eps, nb_iter=nb_iter, eps_iter=eps_iter, rand_init=rand_init, 
            clip_min=clip_min, clip_max=clip_max, targeted=targeted, ord=ord, rand_init_type=rand_init_type,HN_regular = HN_regular,noise_eps=noise_eps)
