...
csv_name =  "target-csv-{}{}-{}{}{}{}{}-{}randint-{}{}{}{}{}-{}target-{}.csv".format(args.agg_type, kex, geps, ghn, args.gattack, args.gattack_loss, args.gattack_iter, ran, 
                                                           seps, shn, args.sattack, args.sattack_loss, args.sattack_iter, sat, suffix)
...
sattack =  HNLinfPGDAttack(model, LOSS[args.sattack_loss], eps=args.sattack_eps, nb_iter=args.sattack_iter, 
                           ...
                           rand_init=False,HN_regular=args.sattack_HN,noise_eps=args.sattack_HNeps)
...
def target_eval(model,dataloader,attack,device,eval_attack=None):
    ...
    for item in dataloader:
        ...
        x_tmp = [x[i].repeat(10,1,1,1) for i in range(len(x))]
        x_repeat = torch.cat(x_tmp,dim=0)

        x_adv_tmp = [x_adv[i].repeat(10,1,1,1) for i in range(len(x_adv))]
        x_adv_repeat = torch.cat(x_adv_tmp,dim=0)

        y_tmp = [j for i in range(len(y)) for j in range(10)]
        ...
        batch_metrics = {'ture_label': torch.cat([i.repeat(10) for i in y]).cpu().numpy(),
                         'y_clean_label': torch.cat([i.repeat(10) for i in y_clean_pred]).cpu().numpy(),
                         'y_adv_label': torch.cat([i.repeat(10) for i in y_adv_pred]).cpu().numpy(),
