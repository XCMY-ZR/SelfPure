...
from core.attacks import  HNLinfPGDAttack, CWLoss
...
class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, seed=None, verbose=True,
        ...
        sattack = HNLinfPGDAttack(model, CWLoss, eps=8/255, nb_iter=20, 
                           eps_iter=2/255, clip_min=0.0, clip_max=1.0, targeted=True, 
                           rand_init=False,HN_regular=True,noise_eps=4/255)
        self.advsm = SelfPure(attackforsmooth=sattack,device=self.device,keep_original_x=True)
        ...
        with torch.no_grad():
            ...
            for attack in attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    # adv_curr = self.advsm.agg()
                
                    output = self.get_logits(adv_curr)
                    adv_curr = self.advsm.agg(x=adv_curr,y=output)
